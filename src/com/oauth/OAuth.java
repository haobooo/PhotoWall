...
import com.oauth.base.WebViewActivity;
import com.photowall.oauth.util.PlatformAccount;
import com.photowall.oauth.util.PlatformBindConfig;
import com.photowall.oauth.util.ServiceUtils;
import com.photowall.photowallcommunity.LogoActivity;
...
public class OAuth {
	...
	public class VerfierRecivier extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (bundle != null) {
				((LogoActivity) activity).mSpinner.show();
				new Thread() {
					public void run() {
		...
		try {
			...
			if (200 == resposne.getStatusLine().getStatusCode()) {
				...
				if (200 == resposne.getStatusLine().getStatusCode()) {
					...
					Message msg = Message.obtain(handler, LogoActivity.oauth_sucess, account);
